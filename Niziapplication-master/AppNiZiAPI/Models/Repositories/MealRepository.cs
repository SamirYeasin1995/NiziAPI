...
namespace AppNiZiAPI.Models.Repositories
{
    class MealRepository :IMealRepository
    {
        public async Task<Meal> AddMeal(Meal meal)
        {
            ...
            sqlQuery.Append("INSERT INTO Meal (patient_id, name, kcal,protein,fiber,calcium,sodium,portion_size,weight_unit_id,picture,description) ");
            sqlQuery.Append("VALUES (@PATIENT_ID, @NAME, @KCAL,@PROTEIN,@FIBER,@CALCIUM,@SODIUM,@PORTION_SIZE,@WEIGHT_UNIT_ID,@PICTURE,@DESCRIPTION) ");
            using (conn)
            {
                ...
                sqlCmd.Parameters.Add("@DESCRIPTION", SqlDbType.NVarChar).Value = meal.Description;
            ...
            using (conn)
            {
                ...
                using (SqlCommand cmd = new SqlCommand(text, conn))
                {
                    ...
                    while (reader.Read())
                    {
                        ...
                        meal.Description = (string)reader["description"];
                    }
                }
                conn.Close();
            }
        ...
        public async Task<List<Meal>> GetMyMeals(int patient_id, int minKcal, int maxKcal)
        {
            SqlConnection conn = new SqlConnection(Environment.GetEnvironmentVariable("sqldb_connection"));
            ...
            using (conn)
            {
                ...
                conn.Open();
                var text = $"SELECT * FROM Meal Inner Join WeightUnit On  meal.weight_unit_id = WeightUnit.id WHERE meal.kcal BETWEEN {minKcal} AND {maxKcal} and patient_id = '{patient_id}'";
        ...
        public async Task<Meal> PutMeal(Meal meal)
        {
            ...
            sqlQuery.Append("set name = @NAME , kcal = @KCAL,protein= @PROTEIN,fiber = @FIBER,calcium =@CALCIUM,sodium =@SODIUM,portion_size = @PORTION_SIZE,weight_unit_id = @WEIGHT_UNIT_ID,picture = @PICTURE,description = @DESCRIPTION");
